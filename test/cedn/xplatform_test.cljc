...
(defn- read-vectors-file
  ...
  []
  ;; cedn/readers: the file contains #bytes literals, and #inst must
  ;; come back at full precision.
  (edn/read-string
   {:readers cedn/readers}
...
(def cedn-p-compliance-vector
  "Single composite value exercising every CEDN-P type: the scalars,
  every collection kind (including empty ones), and all three tagged
  literals.  Its canonical form is the worked example in the README."
  {:bools       [true false]
   :bytes       #?(:clj  (byte-array [(unchecked-byte 0xde) (unchecked-byte 0xad)
                                      (unchecked-byte 0xbe) (unchecked-byte 0xef)])
                   :cljs (js/Uint8Array. #js [0xde 0xad 0xbe 0xef]))
   :empties     [() [] #{} {}]
...
(def cedn-p-compliance-expected
  ...
  "{:bools [true false] :bytes #bytes \"deadbeef\" :collections {:list (1 2 3) :map {:a 1 :b 2} :set #{1 2 3} :vec [1 2 3]} :doubles [3.141592653589793 0.5 -1.5 0.1 1e-7] :empties [() [] #{} {}] :inst #inst \"1970-01-01T00:00:00.000000000Z\" :ints [0 1 -1 42 -7 9007199254740991] :keywords [:foo :ns/bar] :nested {:a [1 #{:x :y}] :b \"hello\"} :nil-val nil :strings [\"\" \"hello\" \"café\" \"a\\tb\" \"a\\nb\" \"a\\\"b\" \"a\\\\b\" \"\\u0000\"] :symbols [foo ns/bar] :uuid #uuid \"f81d4fae-7dec-11d0-a765-00a0c91e6bf6\"}")

(def cedn-p-compliance-hex
  "Golden UTF-8 hex — identical on all platforms."
  "")

(deftest cedn-p-compliance-idempotent-test
  ;; §1.2.3: read ∘ canonicalize is an idempotent projection. Feeding
  ;; canonical output back through the pipeline must be a no-op, for
  ;; every type at once — that is what makes the form canonical.
  (testing "canonical → read → canonical is a fixed point"
    (let [s1 (cedn/canonical-str cedn-p-compliance-vector)
          v2 (edn/read-string {:readers cedn/readers} s1)
          s2 (cedn/canonical-str v2)
          v3 (edn/read-string {:readers cedn/readers} s2)
          s3 (cedn/canonical-str v3)]
      (is (= s1 s2) "second pass differs from the first")
      (is (= s2 s3) "third pass differs from the second")
      (is (cedn/canonical? s1) "canonical output must report as canonical")
      (is (= s1 cedn-p-compliance-expected)))))
